...
usage="Script to qsub jobs to evaluate existing VAE or GroupEnc embedding(s)

$(basename "$0") [-h] [-d dataset] [-n dataset_name] [-l target_dim] [-k group_size] [-r fpath_res] [-o fpath_logs] [-c fname_script] [-v fname_venv] [-m memory] [-e epochs] [-s runs]
...
where:
    ...
    -c --fname_script   path to embed.py script (default: './score.py')
    -v --fname_venv     path to an existing venv (virtual environment) with scoring script dependencies (default: './venv_score')
...
FPATH_LOGS="./logs"
FNAME_SCRIPT="./score.py"
FPATH_VENV="./venv_score"
MEMORY=140
EPOCHS=500
RUNS=5

optspec=":h:d:n:k:l:r:o:c:v:m:e:s:-:"
...
do
	...
	er="${FPATH_LOGS}/${n}Score_ERROR.txt"
	ou="${FPATH_LOGS}/${n}Score_OUTPUT.txt"
	echo "qsubbing ${n}"
	cat <<EOS | qsub -l nodes=1,mem=${MEMORY}gb,walltime=2:00:00 -e $er -o $ou -N $n
source ${FPATH_VENV}/bin/activate
mkdir -p $FPATH_RES
python $FNAME_SCRIPT -f $DATASET -d $DATASET_NAME -r $FPATH_RES -k $k -l $LATENT_DIM -e $EPOCHS -i $i
EOS	
done
